...
pub struct Observation<T: TimeZone, K: Clone> {
    pub interval: Hour<T>,
    pub value: K,
}

pub struct TimeSeries<T: TimeZone, K: Clone>(Vec<Observation<T,K>>); 

impl<T: TimeZone, K: Clone> TimeSeries<T, K> {
...
impl<T: TimeZone,K: Clone> FromIterator<Observation<T,K>> for TimeSeries<T,K> {
...
mod tests {
    ...
    fn push_panic() -> TimeSeries<Utc, bool> {
        let mut ts: TimeSeries<Utc, bool> = TimeSeries::new();
        ts.push(Observation {
            interval: Hour {
                start: Utc.with_ymd_and_hms(2022, 1, 1, 5, 0, 0).unwrap(),
            },
            value: true,
        });
        ts.push(Observation {
    ...
    fn test_timeseries_iter() {
        ...
        ts.push(Observation {
            interval: Hour {
                start: Utc.with_ymd_and_hms(2022, 1, 1, 1, 0, 0).unwrap(),
            },
            value: false,
        });
        ts.push(Observation {
            interval: Hour {
                start: Utc.with_ymd_and_hms(2022, 1, 1, 2, 0, 0).unwrap(),
            },
            value: true,
        });
        let res: TimeSeries<Utc,bool>  = ts.iter()
