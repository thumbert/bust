...
impl ApiQuery {
    pub fn to_query_filter(&self) -> QueryFilter {
        QueryFilter {
            ...
            zoned_gte: self.zoned_gte.clone(),
            zoned_lt: self.zoned_lt.clone(),
        }
    }
}
...
mod api_tests {
    ...
    async fn test_get_data_api() {
        let data = web::Data::new(ProdDb::hq_fuel_mix());
        ...
        let uri = format!("/hq/fuel_mix?{}&_limit=5", params);
