...
use crate::db::isone::mis::sd_dacleared::*;
use crate::utils::lib_duckdb::open_with_retry;

#[get("/mis/sd_dacleared/tab0")]
pub async fn get_data_api(
    query: web::Query<ApiQuery>,
    data: web::Data<SdDaclearedArchive>,
) -> impl Responder {
...
struct ApiQuery {
    ...
    pub bid_offer_type_in: Option<String>,
...
impl ApiQuery {
    pub fn to_query_filter(&self) -> QueryFilter {
        QueryFilter {
            ...
            bid_offer_type: self.bid_offer_type,
            bid_offer_type_in: self.bid_offer_type_in.as_ref().map(|s| {
                s.split(',')
                    .map(|v| v.trim().parse::<BidOfferType>().unwrap())
                    .collect()
            }),
        }
    }
...
mod api_tests {
    ...
    async fn test_get_data_api() {
        let data = web::Data::new(ProdDb::sd_dacleared());
        ...
        let uri = format!("/mis/sd_dacleared/tab0?{}&_limit=5", params);
