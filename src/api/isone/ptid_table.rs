use std::time::Duration;
use actix_web::{get, web, HttpResponse, Responder};
use serde::Deserialize;
use duckdb::AccessMode;

use jiff::civil::Date;

use crate::utils::lib_duckdb::open_with_retry;
use crate::db::isone::ptid_table_archive::*;

#[get("/isone/ptid_table")]
pub async fn get_data_api(query: web::Query<ApiQuery>, data: web::Data<IsonePtidTableArchive>) -> impl Responder {
    let conn = open_with_retry(
        &data.duckdb_path,
        8,
        Duration::from_millis(25),
        AccessMode::ReadOnly,
    );
    if conn.is_err() {
        return HttpResponse::InternalServerError().body(format!(
            "Error opening DuckDB database at {}: {}",
            data.duckdb_path,
            conn.err().unwrap(),
        ));
    }
    let conn = conn.unwrap();

    let query_filter = query.to_query_filter();
    match get_data(&conn, &query_filter, query._limit) {
        Ok(records) => {
            if records.len() > 100_000 {
                HttpResponse::BadRequest()
                    .body(format!("Query returned {} records, only a max of 100,000 are allowed.  Please narrow your query.", records.len()))
            } else {
                HttpResponse::Ok().json(records)
            }
        }
        Err(e) => HttpResponse::InternalServerError().body(format!("Error querying data: {}", e)),
    }
}

#[derive(Debug, Deserialize)]
struct ApiQuery {
    pub node_type: Option<NodeType>,
    pub node_type_in: Option<String>,
    pub activated_on: Option<Date>,
    pub activated_on_in: Option<String>,
    pub activated_on_gte: Option<Date>,
    pub activated_on_lte: Option<Date>,
    pub deactivated_on: Option<Date>,
    pub deactivated_on_in: Option<String>,
    pub deactivated_on_gte: Option<Date>,
    pub deactivated_on_lte: Option<Date>,
    pub _limit: Option<usize>,
}

impl ApiQuery {
    pub fn to_query_filter(&self) -> QueryFilter {
        QueryFilter {
            node_type: self.node_type,
            node_type_in: self.node_type_in.as_ref().map(|s| s.split(',').map(|v| v.trim().parse::<NodeType>().unwrap()).collect()),
            activated_on: self.activated_on,
            activated_on_in: self.activated_on_in.as_ref().map(|s| {s.split(',').map(|v| v.trim().parse::<Date>().unwrap()).collect()}),
            activated_on_gte: self.activated_on_gte,
            activated_on_lte: self.activated_on_lte,
            deactivated_on: self.deactivated_on,
            deactivated_on_in: self.deactivated_on_in.as_ref().map(|s| {s.split(',').map(|v| v.trim().parse::<Date>().unwrap()).collect()}),
            deactivated_on_gte: self.deactivated_on_gte,
            deactivated_on_lte: self.deactivated_on_lte,
        }
    }
}

#[cfg(test)]
mod api_tests {
    use super::*;
    use crate::db::prod_db::ProdDb;
    use actix_web::{test, web, App};

    #[actix_web::test]
    async fn test_get_data_api() {
        let data = web::Data::new(ProdDb::isone_ptid_table_archive());
        let app = test::init_service(App::new().app_data(data.clone()).service(get_data_api)).await;
        let params = QueryFilterBuilder::new().build().to_query_url();
        let uri = format!("/isone/ptid_table?{}&_limit=5", params);
        let req = test::TestRequest::get().uri(&uri).to_request();
        let resp = test::call_service(&app, req).await;
        assert!(resp.status().is_success());
        let rs: Vec<Record> = test::read_body_json(resp).await;
        assert_eq!(rs.len(), 5);
    }
}
