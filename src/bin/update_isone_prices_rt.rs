...
use bust::{
    db::{isone::lib_dam::is_rtlmp_published, prod_db::ProdDb},
...
/// Run this job every day at 5:30PM
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for i in 2..6 {
        ...
        if is_rtlmp_published(date).unwrap() {
            let file = archive.filename(&date) + ".gz";
            if !Path::new(&file).exists() {
                match archive.download_file(date) {
                    Ok(_) => info!(
                        "Downloaded ISONE RT LMP hourly prices file for {} successfully",
                        date
                    ),
                    Err(e) => error!("{:?}", e),
                }
